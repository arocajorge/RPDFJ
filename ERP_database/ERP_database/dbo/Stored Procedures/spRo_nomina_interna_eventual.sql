...
CREATE PROCEDURE [dbo].[spRo_nomina_interna_eventual] (
@IdEmpresa int,
@IdNomina numeric,
...
select    @IdRubro_calculado= idrubro_dias_trabajados  from ro_rubros_calculados where IdEmpresa=@IdEmpresa
...
GROUP BY emp.IdEmpresa, emp.IdEmpleado,contr.FechaFin, contr.FechaInicio, 
                         contr.EstadoContrato

----------------------------------------------------------------------------------------------------------------------------------------------
-------------calculando dias efectivos por empleado-----------------------------------------------------------------------------------------<
----------------------------------------------------------------------------------------------------------------------------------------------

select    @IdRubro_calculado= idrubro_dias_efectivos  from ro_rubros_calculados where IdEmpresa=@IdEmpresa
select @Orden=ru_orden_rol_general from ro_rubro_tipo where IdEmpresa=@IdEmpresa and IdRubro=@IdRubro_calculado
insert into ro_rol_detalle
(IdEmpresa,	IdNominaTipo,IdNominaTipoLiqui,IdPeriodo,									IdEmpleado,			IdRubro,			Orden,			Valor
,rub_visible_reporte,	Observacion)
...
----------------------------------------------------------------------------------------------------------------------------------------------
select    @IdRubro_calculado= idrubro_sueldo  from ro_rubros_calculados where IdEmpresa=@IdEmpresa
select @Orden=ru_orden_rol_general from ro_rubro_tipo where IdEmpresa=@IdEmpresa and IdRubro=@IdRubro_calculado
insert into ro_rol_detalle
(IdEmpresa,	IdNominaTipo,IdNominaTipoLiqui,IdPeriodo,									IdEmpleado,			IdRubro,			Orden,			Valor
,rub_visible_reporte,	Observacion)
...
select TOP(9000)
@IdEmpresa		,@IdNomina,@IdNominaTipo,@IdPEriodo							,emp.IdEmpleado		,@IdRubro_calculado		,ISNULL( @Orden,0)	,COUNT(emp.IdEmpleado)*14
,1						,'SUELDO'		
FROM            dbo.ro_empleado AS emp LEFT OUTER JOIN
                         dbo.vwro_contrato_activo AS contr ON emp.IdEmpresa = contr.IdEmpresa AND emp.IdEmpleado = contr.IdEmpleado LEFT OUTER JOIN
                         dbo.ro_empleado_x_ro_tipoNomina AS nom ON emp.IdEmpresa = nom.IdEmpresa AND emp.IdEmpleado = nom.IdEmpleado LEFT OUTER JOIN
