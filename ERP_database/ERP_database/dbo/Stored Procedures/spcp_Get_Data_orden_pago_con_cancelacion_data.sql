--exec [dbo].[spcp_Get_Data_orden_pago_con_cancelacion_data] 1,0,99999,'',0,99999,'','admin',0
...
select		@IdUsuario, cp_orden_pago.IdEmpresa, cp_orden_pago.IdTipo_op, cp_orden_pago_det.Referencia AS Expr1, '' AS Expr2, cp_orden_pago.IdOrdenPago, cp_orden_pago_det.Secuencia, cp_orden_pago.IdTipo_Persona, cp_orden_pago.IdPersona, cp_orden_pago.IdEntidad, 
                  ...
                  ISNULL(SUM(cp_orden_pago_cancelaciones.MontoAplicado),0) AS MontoAplicado, ROUND(cp_orden_pago_det.Valor_a_pagar - ISNULL(SUM(cp_orden_pago_cancelaciones.MontoAplicado), 0),2) AS saldo, cp_orden_pago.IdEstadoAprobacion, 
