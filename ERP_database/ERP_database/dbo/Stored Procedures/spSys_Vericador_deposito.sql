...
SELECT     IdEmpresa, IdTipoCbte, IdCbteCble, CodCbteCble, IdPeriodo, cb_Fecha, cb_Valor, cb_Observacion, cb_Secuencia, cb_Estado, cb_Anio, cb_mes, IdUsuario, 
                      IdUsuarioAnu, cb_MotivoAnu, IdUsuarioUltModi, cb_FechaAnu, cb_FechaTransac, cb_FechaUltModi, cb_Mayorizado
FROM         ct_cbtecble
WHERE     (IdEmpresa = @I_IdEmpresa) 
AND (IdCbteCble = @I_IdCbteCble) 
AND (IdTipocbte = @I_IdTipocbte)
...
SELECT      IdEmpresa, IdTipoCbte, IdCbteCble, secuencia, IdCtaCble, IdCentroCosto, IdCentroCosto_sub_centro_costo, dc_Valor, dc_Observacion, dc_Numconciliacion, 
                      dc_EstaConciliado
