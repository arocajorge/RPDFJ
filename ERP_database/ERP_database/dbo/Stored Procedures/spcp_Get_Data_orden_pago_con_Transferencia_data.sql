-- exec spcp_Get_Data_orden_pago_con_Transferencia_data  2,0,'28/02/2010','16/11/2018', 'admin'
...
WHERE       op.IdEmpresa = @i_idempresa and (OP.Estado = 'A') AND (OP_det.IdEstadoAprobacion = 'APRO') AND (arc_tr.IdArchivo = @i_IdArchivo) and OP_det.IdCbteCble_cxp is not null
and (exists(
...
WHERE        (OP.IdEmpresa = @i_idempresa) AND (OP.Estado = 'A') AND (OP_det.IdEstadoAprobacion = 'APRO') and op_det.IdCbteCble_cxp is not null
and not exists(
