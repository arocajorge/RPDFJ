...
SELECT        IdEmpresa, em_nombre, pr_codigo, pr_nombre, Factura, NumRetencion, AVG(co_Por_iva) AS co_Por_iva, AVG(co_valoriva) AS co_valoriva, AVG(co_subtotal_iva) AS co_subtotal_iva, AVG(co_subtotal_siniva) 
						 ...
						 Tiene_retencion, 						 
						 re_tipoRet
FROM            (SELECT        dbo.tb_empresa.IdEmpresa, dbo.tb_empresa.em_nombre, dbo.cp_proveedor.pr_codigo, dbo.cp_proveedor.pr_nombre, dbo.cp_orden_giro.co_serie + '-' + dbo.cp_orden_giro.co_factura AS Factura, 
