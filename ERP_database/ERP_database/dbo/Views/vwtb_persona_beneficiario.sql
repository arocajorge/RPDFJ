...
SELECT        prov.IdEmpresa, 'PROVEE' + '-' + CAST(prov.IdPersona AS varchar(20)) + '-' + CAST(prov.IdProveedor AS varchar(20)) AS IdBeneficiario, 'PROVEE' AS IdTipo_Persona, prov.IdPersona, 
                         prov.IdProveedor AS IdEntidad, prov.pr_codigo AS Codigo, prov.pr_nombre AS Nombre, prov.pr_girar_cheque_a, pers.pe_razonSocial, pers.pe_cedulaRuc, pers.pe_Naturaleza, prov.IdCtaCble_CXP AS IdCtaCble, 
                         ...
                         prov.IdBanco_acreditacion, pers.pe_apellido, pers.pe_nombre, pers.pe_nombreCompleto, pers.IdTipoDocumento, pers.pe_direccion, pers.pe_telefonoCasa, pers.pe_celular, pers.pe_correo
...
SELECT        cli.IdEmpresa, 'CLIENTE' + '-' + CAST(pers.IdPersona AS varchar(20)) + '-' + CAST(cli.IdCliente AS varchar(20)) AS IdBeneficiario, 'CLIENTE' AS IdTipo_Persona, pers.IdPersona, cli.IdCliente, cli.Codigo, 
                         ...
                         pers.IdTipoDocumento, pers.pe_direccion, pers.pe_telefonoCasa, pers.pe_celular, pers.pe_correo
FROM            tb_persona AS pers INNER JOIN
