...
AS
SELECT        CM.IdEmpresa, CM.IdCbteCble, CM.IdTipocbte, CM.cm_Signo, CM.cm_beneficiario, CM.cm_valor, CM.IdTipoMovi, CM.cm_observacion, CM.IdCaja, CM.IdPeriodo, CM.cm_fecha, CM.IdUsuario, CM.IdUsuario_Anu, 
                         CM.FechaAnulacion, CM.Fecha_Transac, CM.Fecha_UltMod, CM.IdUsuarioUltMod, CM.Estado, CM.MotivoAnulacion, CM.IdUsuario_Aprueba, TM.tm_descripcion AS NTipoMov, CM.IdCbteCble_Anu, 
                         CM.IdTipocbte_Anu, CM.CodMoviCaja, C.ca_Descripcion, '' AS ResponsableCaja, CM.IdSucursal, S.Su_Descripcion, CM.IdTipoFlujo, dbo.ba_TipoFlujo.Descricion AS nom_tipoFlujo, CM.IdTipo_Persona, 
                         CM.IdEntidad, dbo.vwtb_persona_beneficiario.Nombre AS nom_Beneficiario, CM.IdPersona, CM.IdPuntoVta, CM.IdRecibo
FROM            dbo.caj_Caja_Movimiento AS CM INNER JOIN
                         ...
                         dbo.vwcaj_caj_Caja AS C ON CM.IdEmpresa = C.IdEmpresa AND CM.IdCaja = C.IdCaja INNER JOIN
                         dbo.tb_sucursal AS S ON CM.IdSucursal = S.IdSucursal AND CM.IdEmpresa = S.IdEmpresa LEFT OUTER JOIN
