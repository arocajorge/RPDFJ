...
FROM     (SELECT cbte_d.IdEmpresa, cbte_d.IdTipoCbte, cbte_d.IdCbteCble, cbte_d.IdCtaCble AS IdCtaCble_Acreedora
                  ...
                  FROM     dbo.cp_nota_DebCre INNER JOIN
                                    dbo.ct_cbtecble_det AS cbte_d ON dbo.cp_nota_DebCre.IdEmpresa = cbte_d.IdEmpresa AND dbo.cp_nota_DebCre.IdTipoCbte_Nota = cbte_d.IdTipoCbte AND dbo.cp_nota_DebCre.IdCbteCble_Nota = cbte_d.IdCbteCble
                  WHERE  (cbte_d.dc_Valor < 0) AND (dbo.cp_nota_DebCre.DebCre = 'D') and
                                    (cbte_d.dc_Valor < 0) AND (dbo.cp_nota_DebCre.DebCre = 'D') AND (EXISTS
                                        (SELECT IdEmpresa
                                         FROM      dbo.cp_proveedor_clase AS cl_pr
                                         WHERE   (IdEmpresa = cbte_d.IdEmpresa) AND (IdCtaCble_CXP = cbte_d.IdCtaCble))
										 or EXISTS
