...
SELECT dbo.fa_factura.IdEmpresa, dbo.fa_factura.IdBodega, dbo.fa_factura.IdCbteVta, dbo.fa_factura.IdSucursal, dbo.fa_factura.CodCbteVta, dbo.fa_factura.vt_tipoDoc, dbo.fa_factura.vt_serie1, dbo.fa_factura.vt_serie2, 
                  ...
                  dbo.fa_factura.IdPeriodo, dbo.fa_factura.vt_anio, dbo.fa_factura.vt_mes, dbo.fa_factura.vt_flete, dbo.fa_factura.vt_interes, dbo.fa_factura.vt_OtroValor1, dbo.fa_factura.vt_OtroValor2, dbo.fa_factura.IdUsuario, 
                  dbo.fa_factura.Fecha_Transaccion, dbo.fa_factura.IdUsuarioUltModi, dbo.fa_factura.Fecha_UltMod, dbo.fa_factura.IdUsuarioUltAnu, dbo.fa_factura.Fecha_UltAnu, dbo.fa_factura.MotivoAnulacion, dbo.fa_factura.nom_pc, 
                  dbo.fa_factura.ip, dbo.fa_factura.Estado, dbo.tb_sucursal.Su_Descripcion, dbo.tb_bodega.bo_Descripcion, 0 AS Secuencia, dbo.fa_Vendedor.Ve_Vendedor, dbo.tb_persona.pe_nombreCompleto, dbo.fa_factura.vt_autorizacion, 
                  dbo.tb_persona.IdTipoDocumento, dbo.tb_persona.pe_cedulaRuc, dbo.fa_factura.IdCaja, dbo.fa_factura.IdEmpresa_nc_anu, dbo.fa_factura.IdSucursal_nc_anu, dbo.fa_factura.IdBodega_nc_anu, dbo.fa_factura.IdNota_nc_anu, 
                  dbo.fa_factura.vt_seguro, ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.SubTotal_0, 0) + ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.SubTotal_Iva, 0) AS vt_Subtotal, ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.vt_iva, 0) 
                  AS vt_iva, ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.SubTotal_0, 0) AS SubTotal_0, ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.SubTotal_Iva, 0) AS SubTotal_Iva, ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.vt_total, 0) 
                  AS vt_total, dbo.fa_factura.IdPuntoVta, ct.cbte, ISNULL(cobro.valor_cobro, 0) AS valor_cobro, ROUND(ROUND(ISNULL(dbo.vwfa_factura_subtotal_iva_0_totales.vt_total, 0), 2) - ROUND(ISNULL(cobro.valor_cobro, 0), 2), 2) AS vt_saldo, 
                  formas_pago.IdFormaPago, dbo.fa_formaPago.nom_FormaPago, formas_pago.cant_forma_pago, dbo.fa_factura.Fecha_Autorizacion
FROM     dbo.fa_formaPago INNER JOIN
                  ...
                  dbo.fa_factura.IdBodega = dbo.vwfa_factura_subtotal_iva_0_totales.IdBodega AND dbo.fa_factura.IdCbteVta = dbo.vwfa_factura_subtotal_iva_0_totales.IdCbteVta LEFT OUTER JOIN
                      (SELECT f.vt_IdEmpresa, f.vt_IdSucursal, f.vt_IdBodega, f.vt_IdCbteVta, MAX(f.ct_IdCbteCble) AS cbte
                       ...
                       GROUP BY f.vt_IdEmpresa, f.vt_IdSucursal, f.vt_IdBodega, f.vt_IdCbteVta) AS ct ON dbo.fa_factura.IdEmpresa = ct.vt_IdEmpresa AND dbo.fa_factura.IdSucursal = ct.vt_IdSucursal AND dbo.fa_factura.IdBodega = ct.vt_IdBodega AND 
                  dbo.fa_factura.IdCbteVta = ct.vt_IdCbteVta LEFT OUTER JOIN
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[83] 4[3] 2[1] 3) )"
      End
