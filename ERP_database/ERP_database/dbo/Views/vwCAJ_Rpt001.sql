...
SELECT        caj.IdEmpresa, caj.IdCbteCble, caj.IdTipocbte, tip_cbt_cbl.tc_TipoCbte AS Tipo_Cbte, caj_1.ca_Codigo AS cod_caja, caj_1.ca_Descripcion AS Caja, 
                         sucu.Su_Descripcion AS Sucursal, CASE caj.cm_Signo WHEN '+' THEN 'INGRESO' WHEN '-' THEN 'EGRESO' END AS Tipo, caj.cm_beneficiario AS Beneficiario, 
                         caj_d.cr_Valor AS Valor, caj.cm_fecha AS Fecha, caj_tip_mov.tm_descripcion AS Tipo_Movi_Caja, caj_d.IdCobro_tipo, caj_d.cr_Banco AS Banco, 
                         caj_d.cr_NumDocumento AS Num_Documento, caj.cm_observacion AS Observacion, dbo.tb_Calendario.IdCalendario, dbo.tb_Calendario.AnioFiscal, 
...
FROM            dbo.caj_Caja AS caj_1 INNER JOIN
                         ...
                         caj.IdTipocbte = caj_d.IdTipocbte LEFT OUTER JOIN
                         dbo.tb_sucursal AS sucu ON caj.IdEmpresa = sucu.IdEmpresa AND caj.IdSucursal = sucu.IdSucursal
GO
