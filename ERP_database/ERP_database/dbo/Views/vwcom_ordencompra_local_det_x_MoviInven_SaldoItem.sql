...
SELECT     B.IdEmpresa, B.IdSucursal, B.IdOrdenCompra, B.Secuencia, B.IdProducto, ISNULL(AVG(B.do_Cantidad), 0) AS do_CantidadOC, ISNULL(SUM(D.dm_cantidad), 0) 
                      AS dm_cantidad_Inv, AVG(B.do_Cantidad) - ISNULL(SUM(D.dm_cantidad), 0) AS SaldoxRecibir, dbo.cp_proveedor.pr_nombre, 
...
FROM         dbo.com_ordencompra_local INNER JOIN
                      ...
                      B.Secuencia = C.ocd_Secuencia
GROUP BY B.IdEmpresa, B.IdSucursal, B.IdOrdenCompra, B.Secuencia, B.IdProducto, dbo.cp_proveedor.pr_nombre, dbo.com_ordencompra_local.IdProveedor, 
