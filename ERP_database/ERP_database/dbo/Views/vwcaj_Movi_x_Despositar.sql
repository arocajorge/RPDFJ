...
AS
SELECT        B.IdEmpresa, B.IdSucursal, B.IdCobro, B.IdCobro_tipo, B.IdCliente, caj_Caja_Movimiento_det.cr_Valor AS cr_TotalCobro, B.cr_fecha, B.cr_fechaCobro, 
...
UNION
SELECT        B.IdEmpresa, A.IdSucursal, B.IdCbteCble AS IdCobro, CASE WHEN C.IdCobro_tipo IS NULL 
                         THEN B.CodMoviCaja WHEN cm_Signo = '+' THEN 'ING' WHEN cm_Signo = '-' THEN 'EGR' END AS IdCobro_tipo, 0 AS IdCliente, C.cr_Valor, B.cm_fecha, 
                         C.cr_fechaDocu AS Fecha_Cobro, B.cm_observacion, C.cr_Banco, C.cr_cuenta, NULL AS Tarjeta, C.cr_NumDocumento, B.Estado, NULL AS Recibo, A.Su_Descripcion, 
                         B.cm_beneficiario, C.cr_fechaDocu, E.IdCaja, E.ca_Descripcion, F.IdTipoMovi, F.tm_descripcion, B.cm_Signo, B.IdUsuario, B.IdCbteCble AS IdCbteCble_MoviCaja, 
...
FROM            caj_Caja AS E INNER JOIN
                         ...
                         caj_Caja_Movimiento_Tipo AS F ON B.IdTipoMovi = F.IdTipoMovi INNER JOIN
                         tb_sucursal AS A ON B.IdEmpresa = A.IdEmpresa AND B.IdSucursal = A.IdSucursal
WHERE        
B.Estado='A'
and NOT EXISTS
