...
as
SELECT        ISNULL(ROW_NUMBER() over(order by C.IdEmpresa),0) AS IdRow, C.IdEmpresa, C.IdSucursal, C.IdBodega, C.IdMovi_inven_tipo, C.IdNumMovi, C.Secuencia, C.mv_tipo_movi, C.IdProducto, C.dm_cantidad, C.dm_stock_ante, 
                         C.dm_stock_actu, C.dm_observacion, C.mv_costo, A.Su_Descripcion, B.bo_Descripcion, D.pr_codigo, D.pr_descripcion, F.IdMarca, F.Descripcion, E.IdCategoria, 
                         E.ca_Categoria, E.RutaPadre, H.cm_observacion, H.cm_fecha, H.CodMoviInven, dbo.in_movi_inven_tipo.Codigo AS CodTipoMoviInven, 
