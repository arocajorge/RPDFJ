...
SELECT        cabfac.IdEmpresa, cabfac.IdSucursal, cabfac.IdBodega, cabfac.vt_tipoDoc, SUBSTRING(cabfac.vt_tipoDoc, 1, 2) 
                         ...
                         dbo.tb_persona.pe_nombreCompleto AS nombreCliente, cabfac.vt_fecha, ROUND(SUM(detfac.vt_total) + AVG(cabfac.vt_flete) + AVG(cabfac.vt_interes) 
                         + AVG(cabfac.vt_OtroValor1) + AVG(cabfac.vt_OtroValor2), 2) AS vt_total, ROUND(SUM(detfac.vt_total) + AVG(cabfac.vt_flete) + AVG(cabfac.vt_interes) 
                         + AVG(cabfac.vt_OtroValor1) + AVG(cabfac.vt_OtroValor2), 2) - ROUND(ISNULL(AVG(dbo.vwcxc_total_cobros_x_Docu.dc_ValorPago), 0), 2) AS Saldo, 
