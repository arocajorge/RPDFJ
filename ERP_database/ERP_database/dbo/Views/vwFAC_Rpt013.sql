...
SELECT        suc.IdEmpresa, tb_empresa.em_nombre AS nom_empresa, suc.IdSucursal, suc.Su_Descripcion AS nom_sucursal, fa.IdCliente, 
                         ...
                         fa.vt_tipoDoc + '-' + fa.vt_serie1 + '-' + fa.vt_serie2 + '-' + fa.vt_NumFactura AS Documento, SUM(fa_d.vt_total) + AVG(fa.vt_flete) + AVG(fa.vt_interes) 
                         + AVG(fa.vt_seguro) + AVG(fa.vt_OtroValor1) + AVG(fa.vt_OtroValor2) AS Debito, 0 AS Credito, 0 AS Saldo, fa.vt_Observacion
