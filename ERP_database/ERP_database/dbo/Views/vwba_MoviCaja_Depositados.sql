...
as
SELECT        (CASE WHEN B.IdEmpresa IS NULL THEN G.IdEmpresa ELSE B.IdEmpresa END) AS IdEmpresa, (CASE WHEN B.IdSucursal IS NULL 
                         THEN G.IdSucursal ELSE B.IdSucursal END) AS IdSucursal, (CASE WHEN B.IdCobro IS NULL THEN G.IdCbteCble ELSE B.IdCobro END) AS IdCobro, 
                         ...
                         (CASE WHEN A.Su_Descripcion IS NULL THEN
                             (SELECT        Su_Descripcion
                               FROM            tb_sucursal
                               WHERE        IdSucursal = G.IdSucursal AND IdEmpresa = G.IdEmpresa) ELSE A.Su_Descripcion END) AS nSucursal, (CASE WHEN D .pe_nombreCompleto IS NULL 
                         THEN G.cm_beneficiario ELSE D .pe_nombreCompleto END) AS nCliente, B.cr_fechaDocu, E.IdCaja, E.ca_Descripcion, F.IdTipoMovi, F.tm_descripcion, F.tm_Signo, 
