CREATE VIEW XXXDECIMOTERCERO_ANUAL
AS
SELECT A.IdEmpresa, A.pe_nombreCompleto, A.IdEmpleado ,SUM(A.SUELDO_NETO)SUELDO_NETO, SUM(SUELDO_MENOS_FALTA) SUELDO_MENOS_FALTA, SUM(TOTAL_SOBRETIEMPO)TOTAL_SOBRETIEMPO, SUM(VARIABLE)VARIABLE, SUM(DIAS_FALTAS)DIAS_FALTAS, SUM(VALOR_FALTA)VALOR_FALTA, SUM(TOTAL)TOTAL
FROM (
SELECT A.IdEmpresa, A.pe_cedulaRuc, A.pe_nombreCompleto, A.IdPeriodo, SUM(SUELDO_NETO)SUELDO_NETO, ROUND(SUM(SUELDO_NETO) - (SUM(SUELDO_BASE) /30) * SUM(DIAS_FALTAS),2) SUELDO_MENOS_FALTA, SUM(TOTAL_SOBRETIEMPO)TOTAL_SOBRETIEMPO,SUM(VARIABLE)VARIABLE,
SUM(DIAS_FALTAS) DIAS_FALTAS, ROUND((SUM(SUELDO_BASE) /30) * SUM(DIAS_FALTAS),2) VALOR_FALTA,
ROUND(SUM(SUELDO_NETO) - (SUM(SUELDO_BASE) /30) * SUM(DIAS_FALTAS),2)+SUM(TOTAL_SOBRETIEMPO)+SUM(VARIABLE) TOTAL,A.IdEmpleado
FROM (
...
AND E.em_statuS IN('EST_ACT','EST_SUB','EST_PER') --and e.IdEmpleado = @IDEMPLEADO
)A 
GROUP BY A.IdEmpresa, A.pe_cedulaRuc, A.pe_nombreCompleto,A.IdPeriodo,a.IdEmpleado
) A
GROUP BY A.IdEmpresa, A.pe_nombreCompleto, a.IdEmpleado
--ORDER BY A.pe_nombreCompleto