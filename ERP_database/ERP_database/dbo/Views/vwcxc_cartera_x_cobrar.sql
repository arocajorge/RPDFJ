...
SELECT        cabfac.IdEmpresa, cabfac.IdSucursal, cabfac.IdBodega, cabfac.vt_tipoDoc, cabfac.vt_tipoDoc + '-' + cast(cast(cabfac.vt_NumFactura AS int) AS varchar(20)) 
                         ...
                         cabfac.IdCliente, cabfac.vt_fecha, ROUND(SUM(detfac.vt_total) + AVG(cabfac.vt_flete) + AVG(cabfac.vt_interes) + AVG(cabfac.vt_OtroValor1) 
                         + AVG(cabfac.vt_OtroValor2), 2) AS vt_total, ROUND(ROUND(ROUND(SUM(detfac.vt_total), 2) + ROUND(AVG(cabfac.vt_flete), 2) + ROUND(AVG(cabfac.vt_interes), 2) 
                         + ROUND(AVG(cabfac.vt_OtroValor1), 2) + ROUND(AVG(cabfac.vt_OtroValor2), 2), 2) - ROUND(ISNULL(AVG(vwcxc_total_cobros_x_Docu.dc_ValorPago), 0), 2), 2) 
