...
--	set @Fecha_Fin ='2020-06-30'
--	set @Idperiodo_inicio =202006
--		set @Idperiodo_fin =202006


declare @Idperiodo_i int
declare @Idperiodo_f int

select @Idperiodo_i=IdPeriodo from Fj_servindustrias.ro_planificacion_x_ruta_x_empleado 
where IdEmpresa=@IdEmpresa and IdNomina_tipo_Liq=@IdNomina_tipo and IdPeriodo=@Idperiodo_inicio


select @Idperiodo_f=IdPeriodo from Fj_servindustrias.ro_planificacion_x_ruta_x_empleado 
where IdEmpresa=@IdEmpresa and IdNomina_tipo_Liq=@IdNomina_tipo and IdPeriodo=@Idperiodo_fin
...
FROM            dbo.ro_Division INNER JOIN
                         ...
                         dbo.ro_empleado_x_ro_tipoNomina AS ExN ON Em.IdEmpleado = ExN.IdEmpleado AND Em.IdEmpresa = ExN.IdEmpresa INNER JOIN
...
FROM            dbo.ro_Division INNER JOIN
                         ...
                         dbo.ro_turno AS Turn ON Ma.IdEmpresa = Turn.IdEmpresa AND Ma.IdTurno = Turn.IdTurno INNER JOIN
                         dbo.ro_Departamento AS Dep ON Em.IdDepartamento = Dep.IdDepartamento AND Dep.IdEmpresa = Em.IdEmpresa ON dbo.ro_Division.IdDivision = Em.IdDivision AND dbo.ro_Division.IdEmpresa = Em.IdEmpresa INNER JOIN
                         dbo.ro_area ON Em.IdArea = dbo.ro_area.IdArea AND Em.IdEmpresa = dbo.ro_area.IdEmpresa
						 WHERE Ma.IdEmpresa=@IdEmpresa
						 AND Ma.IdNomina_Tipo=@IdNomina_tipo
						 AND Ma.es_fecha_registro between @Fecha_Inicio and @Fecha_Fin
						 and  Em.IdEmpresa=@IdEmpresa
