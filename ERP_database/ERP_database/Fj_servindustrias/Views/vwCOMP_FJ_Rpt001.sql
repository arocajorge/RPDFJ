CREATE VIEW [Fj_servindustrias].[vwCOMP_FJ_Rpt001]
AS
SELECT OC.IdEmpresa, OC.IdSucursal, OC.IdOrdenCompra, OC.IdProveedor, OC.oc_NumDocumento, OC.Tipo, OC.IdTerminoPago, OC.oc_plazo AS Plazo, OC.oc_fecha AS Fecha, OC.oc_flete AS Flete, OC.oc_observacion AS Observacion, 
                  OC.Estado, OC.IdSolicitante, OC.IdComprador, OC.IdDepartamento, depar.de_descripcion AS departamento, OC_det.Secuencia, OC_det.IdProducto, OC_det.do_Cantidad AS cantidad, OC_det.do_precioCompra AS precio, 
                  OC_det.do_porc_des AS por_desc, OC_det.do_descuento AS valor_descuento, OC_det.do_subtotal AS subtotal, OC_det.do_iva AS iva, OC_det.do_total AS total, OC_det.do_peso AS peso, Prod.pr_codigo AS cod_producto, 
                  Prod.pr_descripcion AS nom_producto, sucu.Su_Descripcion AS sucursal, empr.em_nombre AS empresa, empr.em_ruc AS ruc_empresa, empr.em_logo AS logo_empresa, prove.pr_nombre AS nom_proveedor, 
                  per_prov.pe_cedulaRuc AS ced_ruc_provee, per_prov.pe_direccion AS direc_provee, per_prov.pe_telefonoOfic AS telef_provee, dbo.in_UnidadMedida.Descripcion AS NomUnidad, dbo.com_comprador.Descripcion AS Nom_comprador, 
                  OC_det.IdCentroCosto, OC_det.IdCentroCosto_sub_centro_costo, dbo.ct_centro_costo.Centro_costo AS nom_centro_costo, dbo.ct_centro_costo_sub_centro_costo.Centro_costo AS nom_sub_centro_costo, 
                  OC_det.do_observacion AS Detalle_x_Items, OC_det.IdPunto_cargo, dbo.ct_punto_cargo.nom_punto_cargo, empr.em_direccion, dbo.com_solicitante.nom_solicitante, dbo.com_Motivo_Orden_Compra.Descripcion, empr.em_telefonos, 
                  'fjservindustrias@gmail.com' AS mail_empresa, sucu.Su_Direccion AS Direccion_sucu, sucu.Su_Telefonos AS Telef_Sucursal, prove.contacto AS Contacto_Prov, per_prov.pe_correo AS mail_prove, OC.IdEstadoAprobacion_cat, 
                  dbo.com_TerminoPago.Descripcion AS Nom_TerminoPago
FROM     dbo.com_solicitante RIGHT OUTER JOIN
                  ...
                  dbo.ro_Departamento AS depar ON OC.IdEmpresa = depar.IdEmpresa AND OC.IdDepartamento = depar.IdDepartamento LEFT OUTER JOIN
